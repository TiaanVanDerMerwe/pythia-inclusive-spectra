...
int main(int argc, char* argv[]) {
  ...
  //
  // 2 : Hard QCD only.
  //     Fixed pTHat bins defined via Main:subrun in an external .cmnd file.
  //
  // 3 : Hard QCD only.
  ...
  //     Inclusive low-pT physics, used for soft reference studies.
  //     pTHat bins are for analysis only.
  //
  ...
  std::string fname = "logs/pythia_log_" + std::to_string((int)mode) + ".log";
  ...
  int nEventTotal = 10000000;
  // Calculate events per bin
  int nEvent = nEventTotal / nBin;
  ...
  for (int iBin = 0; iBin < nBin; ++iBin) {
    ...
    // Mode 2: subruns stored in the main322.cmnd file.
      ...
      pythia.readFile("main322.cmnd", iBin);
    }
